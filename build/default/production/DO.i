...
uint8 DO_Init(DO_t DO ,volatile uint8* port ,volatile uint8* dir,uint8 pinN ,LEDState_t state)
{
    ...
    GPIO_Init_Pin(dir,pinN,(0));
    ((*port)=(*port & ~(1<<pinN))|(state<<pinN));
}
uint8 DO_GetState(DO_t DO)
{
    return ((*(DOArray[DO].portRegPtr) >> DOArray[DO].pin)& 1);
}
void DO_SetState(DO_t DO,LEDState_t state)
{
    ((*(DOArray[DO].portRegPtr))=(*(DOArray[DO].portRegPtr) & ~(1<<DOArray[DO].pin))|(state<<DOArray[DO].pin));
}
void DO_update(void)
{
